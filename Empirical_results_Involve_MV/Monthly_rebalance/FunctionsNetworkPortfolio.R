...
linfun3=function(Sn,b,lambda,a1=1)
{
  ...
  A1=cbind(-Sn)
  A2=-A1
  A=rbind(A0,A1,A2)   # Inequality left
...
linfun5=function(Sn,b,lambda,a1=1,indx=1)
{
  #equivalent to solving   min 1'u
  #such that		u+x>=0
  #             u-x>=0
  ...
  A0 = toeplitz(a)
  A0[upper.tri(A0)] = -A0[upper.tri(A0)]
  A1=cbind(-Sn, matrix(0, p, p))
  A2=-A1
  A=rbind(A0,A1,A2)
  rhs=c(rep(0,2*p),c(-b,b)-lambda*rep(1,p))
  C=rep(c(1,0),c(p,p))
  a=rep(0,2*p)
  a[c(indx,indx+p)]=1
  EE=diag(a)
  ...
  FF=a
  solvetheta=linp(E=EE,F=FF,
                  G=A,H=rhs,Cost=C,ispos=FALSE)$X[(1):(p)]
  return(solvetheta)
}
...
estimate_rollingwindow_dantzig_lambda_eigenvectorcentrality = function(file_name = "SP500 securities_up_20230306.csv", 
  ...
  # rolling window
  W<-list()
  for(t in 0: (floor((1857-windowsize)/22)-1)){
    W[[(t+1)]]=returnstd[(1+t*22):((windowsize+22)+t*22),]
  }
  W_in<-list()
  W_out<-list()
  for(t in 0: (floor((1857-windowsize)/22)-1)){
    W_in[[(t+1)]]=W[[t+1]][c(1:windowsize),]
    W_out[[(t+1)]]=W[[t+1]][c((windowsize+1):(windowsize+22)),]
  }
  T.windows<-length(W)
  ...
  save(lmd.EC.Dantzig.list, file=paste0("Dantzig_lambda_rolling_window_windowsize",windowsize,".RData"))
...
estimate_rollingwindow_dantzig_eigenvectorcentrality = function(file_name = "SP500 securities_up_20230306.csv", 
  ...
  load(paste0("Dantzig_lambda_rolling_window_windowsize",windowsize,".RData"))
  ...
  save(eigenvector_centrality,file = paste0("eigenvector_centrality_WS",windowsize,"_20250117.RData"))
...
estimate_dantzig_lambda1_portfolio_parameter(file_name = "SP500 securities_up_20230306.csv", windowsize=375){
  ...
  save(dantzig_lambda1_portfolio_parameter,file = paste0("dantzig_lambda1_portfolio_parameter_WS",windowsize,"_20250117.RData"))
...
estimate_dantzig_lambda2_portfolio_parameter(file_name = "SP500 securities_up_20230306.csv", windowsize=375){
  ...
  save(dantzig_lambda2_portfolio_parameter,file = paste0("dantzig_lambda2_portfolio_parameter_WS",windowsize,"_20250117.RData"))
  
  toc()
}

##### CV for tuning lambda in estimation Sigma^-1 phi, using the first "windowsize"(500/750/1000) data ######
estimate_dantzig_lambda3_portfolio_parameter(file_name = "SP500 securities_up_20230306.csv", windowsize=375){
  ...
  load(paste0("eigenvector_centrality_WS",windowsize,"_20250117.RData"))
  EC_DS = eigenvector_centrality$EC_DS
  ...
  save(dantzig_lambda3_portfolio_parameter,file = paste0("dantzig_lambda3_portfolio_parameter_WS",windowsize,"_20250117.RData"))
...
estimate_glasso_rho_portfolio_parameter(file_name = "SP500 securities_up_20230306.csv", windowsize=375){
  ...
  save(glasso_rho_portfolio_parameter,file = paste0("glasso_rho_portfolio_parameter_WS",windowsize,"_20250117.RData"))
