...
plot_dantzig <- ggplot(plot_data, aes(x = x)) +
  geom_line(aes(y = median_lambda1_750, color = "median(lambda_1) (WS750)"), size = 1) +  
  scale_color_manual(values = c("median(lambda_1) (WS750)" = "red")) +  # Explicit colors
...
# Save the plot
ggsave(filename = "Dantzig_hyper_portfolio_lambda1_moving_median_rollingwindow_750.png",
       plot = plot_dantzig,
       width = 10,
       height = 6,
       dpi = 300)
