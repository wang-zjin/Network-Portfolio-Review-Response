...
# load data
prices<-read_excel("Price_MV.xlsx",sheet="PRICE", na = "#N/A N/A")
# Identify columns with no NAs, excluding the 'Dates' column
non_na_cols <- sapply(prices[,-1], function(col) all(!is.na(col)))
# Include the 'Dates' column and columns with no NAs
prices_clean <- prices[, c(TRUE, non_na_cols)]
# Create the zoo object
ZOO <- zoo(prices_clean[,-1], order.by = as.Date(prices_clean$Dates, format = '%Y-%m-%d'))
...
#   lambda.grid=seq(0,1.5,length=101)[2:101]
...
#     lmd=lambda.grid[i]
#     print(i)
#     lin.train=linfun5(Sn=cov.train, b=mu.train, lambda=lmd, a1=max(abs(eigen(A.train)$vector[1,])), indx=which.max(abs(eigen(A.train)$vector[1,])))
#     #lin.train=linfun5(Sn=cov.train, b=mu.train, lambda=lmd, a1=1, indx=which.max(abs(eigen(A.train)$vector[1,])))
#     if(!(all(lin.train==0))){
...
#   print(t)
#   A = C_in[[t]]-diag(1,p,p)
#   EC_DS[[t]] =linfun5(Sn = A-diag(max(eigen(A)$value),p,p),
#                       b = rep(0,p),
#                       lambda = lmd.EC.Dantzig + 4,
#                       a1 = max(abs(eigen(A)$vector[1,])),
#                       indx = which.max(abs(eigen(A)$vector[1,]))
#   )
...
# save(eigenvector_centrality,file = "eigenvector_centrality_20241120.RData")
...
#             "theta2"=theta2,
#             "theta3"=theta3)
# save(theta,file="theta_Dantzig_20241120.RData")
load("theta_Dantzig_20241120.RData")
theta1=theta$theta1
theta2=theta$theta2
theta3=theta$theta3
...
for (i in 1:length(quantl)) {
  ...
  for(t in 1: length(W_in)){
    ## compute global minimum variance portfolio ##
    net.gmin.port = network.3constraint.portfolio(EC_DS[[(t)]],ER_in[[t]], COV_in[[(t)]], quantile(EC_DS[[(t)]],quantl[i])+0.001, mean(ER_in[[t]]), FALSE)
    w[[(t)]] = net.gmin.port$weights
    aus<-as.matrix(repmat(w[[(t)]],22,1)*W_out[[t]])
    cumureturn_temporal[[t]]<-rowSums(aus)
  }
...
save(Portfolio.Scenario,file="Portfolios_20241120.RData")
