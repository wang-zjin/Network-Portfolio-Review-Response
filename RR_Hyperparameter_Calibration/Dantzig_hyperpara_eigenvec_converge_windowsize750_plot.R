...
plot_dantzig <- ggplot(plot_data, aes(x = x)) +
  geom_line(aes(y = lmd_EC_Dantzig, color = "rho^E (WS750)"), size = 1) +  # First line
  geom_line(aes(y = median_rho_750, color = "median(rho^E) (WS750)"), size = 1) +  # Second line
  scale_color_manual(values = c("rho^E (WS750)" = "blue", "median(rho^E) (WS750)" = "red")) +  # Explicit colors
  labs(
    x = "",
...
# Save the plot
ggsave(filename = "Dantzig_hyper_eigenvector_converge_rollingwindow_750.png",
       plot = plot_dantzig,
       width = 10,
       height = 6,
       dpi = 300)
