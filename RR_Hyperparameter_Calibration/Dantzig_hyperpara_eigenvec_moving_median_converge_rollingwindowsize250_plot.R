...
median_rho_250 <- rollapply(
  unlist(lmd.EC.Dantzig.list),
  width = window_size,
  FUN = median,
...
plot_dantzig <- ggplot(plot_data, aes(x = x)) +
  geom_line(aes(y = median_rho_250, color = "median(rho^E) (WS250)"), size = 1) +  
  scale_color_manual(values = c("median(rho^E) (WS250)" = "red")) +  # Explicit colors
...
# Save the plot
ggsave(filename = "Dantzig_hyper_eigenvector_moving_median_converge_rollingwindow_250.png",
       plot = plot_dantzig,
       width = 10,
       height = 6,
       dpi = 300)
