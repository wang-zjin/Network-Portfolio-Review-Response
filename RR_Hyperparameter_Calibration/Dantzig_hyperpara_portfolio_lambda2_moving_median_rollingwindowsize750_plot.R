...
window_size <- 5  # Choose your desired window length
median_lambda2_750 <- rollapply(
  lmd2.EC.Dantzig,
  width = window_size,
  FUN = median,
  align = "right",  # or "center", "left" depending on your needs
  fill = NA       # Fills the initial positions where the window is incomplete
)
...
plot_dantzig <- ggplot(plot_data, aes(x = x)) +
  geom_line(aes(y = median_lambda2_750, color = "median(lambda_2) (WS750)"), size = 1) +  
  scale_color_manual(values = c("median(lambda_2) (WS750)" = "red")) +  # Explicit colors
  scale_y_continuous(limits = c(0, 0.01)) +  # Fix y-axis from 0 to 1
  labs(
    x = "",
    y = "",
    color = "Legend"
...
# Save the plot
ggsave(filename = "Dantzig_hyper_portfolio_lambda2_moving_median_rollingwindow_750.png",
       plot = plot_dantzig,
       width = 10,
       height = 6,
       dpi = 300)
